  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-bold text-gray-900 px-1">Shop by Category</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight">
                Fast Delivery,<br />Fresh Products
              </h1>
            ...
            <div className="relative hidden lg:block">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 transform hover:scale-105 transition-all">
                    <div className="text-4xl mb-3">{f.icon}</div>
                    <p className="text-sm font-bold tracking-tight">{f.label}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/category/fruits" className="text-purple-600 font-semibold text-sm hover:underline">
            View All →
          </Link>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: "🚚", title: "Fast Delivery", desc: "Get your orders delivered in minutes, not hours." },
          { icon: "✨", title: "Fresh Products", desc: "Only the freshest products, handpicked for you." },
          { icon: "💰", title: "Best Prices", desc: "Competitive prices with great deals every day." }
        ].map((f, i) => (
          <div key={i} className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 hover:shadow-md transition-all group">
            <div className="text-4xl mb-4 transform group-hover:scale-110 transition-transform">{f.icon}</div>
